...
ALLOWED_FUNCS = {"floor": math.floor, "ceil": math.ceil, "sqrt": math.sqrt}
ALLOWED_NAMES = {"pi": math.pi, "e": math.e}

def _is_allowed_node(node: ast.AST) -> bool:
    return type(node) in ALLOWED_NODES

def _walk_ok(node: ast.AST) -> bool:
    for n in ast.walk(node):
        if not _is_allowed_node(n):
            return False
        # Restrict function calls to allowed math-like helpers only, no attributes except allowed
...
def safe_eval_expr(expr: str) -> float:
    """
    Avalia expressões matemáticas seguras (+, -, *, /, //, %, **, (), números).
    Também permite funções: floor, ceil, sqrt e constantes pi, e.
    Retorna float; em erro, retorna 0.0.
    """
    if expr is None:
        return 0.0
    expr = str(expr).strip()
    ...
    try:
        ...
        if not _walk_ok(node):
            return 0.0
        compiled = compile(node, "<expr>", "eval")
        return float(eval(compiled, {"__builtins__": {}}, {**ALLOWED_FUNCS, **ALLOWED_NAMES}))
    except Exception:
        return 0.0

# ---------- Core logic (mantém do v6) ----------
...
def value_color(v: float) -> str:
    if float(v) < 40: return "#D9534F"
    if float(v) < 70: return "#F0AD4E"
    return "#5CB85C"
...
# App
# =========================
st.set_page_config(page_title="Temperatura da Pauta — Pro v7", page_icon="🔥", layout="wide")
st.title("🔥 Temperatura da Pauta — Pro v7")
...
with col_a:
    ...
    # Menções Instagram (numérico simples como antes)
    menc_ig = st.number_input("Menções no Instagram", min_value=0, value=120, step=10)

    # Engajamentos Instagram (aceita expressão)
    expr_eng_ig = st.text_input("Engajamentos no Instagram (aceita expressão)", value="3000")
    eng_ig = safe_eval_expr(expr_eng_ig)
    st.caption(f"↳ IG engaj. calculado: {eng_ig:,.0f}".replace(",", "."))

with col_b:
    menc_tw = st.number_input("Menções no Twitter/X", min_value=0, value=80, step=10)

    # Engajamentos Twitter (aceita expressão)
    expr_eng_tw = st.text_input("Engajamentos no Twitter/X (aceita expressão)", value="1800")
    eng_tw = safe_eval_expr(expr_eng_tw)
    st.caption(f"↳ X engaj. calculado: {eng_tw:,.0f}".replace(",", "."))
...
# Donut Score com cores por faixa
...
# Google Trends (skip first 2 rows + cinza)
...
trends_file = st.file_uploader("Envie o CSV bruto do Google Trends (pode manter as 2 linhas iniciais)", type=['csv'])
...
st.markdown("---")
st.caption("Creative Data • Temperatura da Pauta — Pro v7 (engajamentos com expressão + thresholds + Trends)")
